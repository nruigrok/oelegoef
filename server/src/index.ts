...
// Until the real client exists, the site is the interactive mockup (mockup/index.html,
// generated by tools/wrap_mockup.py) — a self-contained page with no API calls. It takes
// precedence over the Toby client build below, which is still the cat game.
const mockupDir = path.join(__dirname, "..", "..", "mockup");
if (fs.existsSync(path.join(mockupDir, "index.html"))) {
  app.use(express.static(mockupDir, { index: "index.html" }));
}
