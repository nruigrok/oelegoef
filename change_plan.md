...
- Decide where it runs. Nothing on this machine currently serves the Toby game, so
  this is a fresh deploy: `npm run build` then `npm run start`, behind a systemd unit
  and a reverse proxy with HTTPS (Web Push and the service worker require a secure
  origin). The AmCAT Caddy already holds ports 80/443 here, so either add a site block
  to that Caddy config or host elsewhere.
- Start with an empty `oelegoef.db`. VAPID keys are generated on first boot and stored
  in SQLite; back up the db file, because losing the keys orphans every subscription.
