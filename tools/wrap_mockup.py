...
SITE_CSS = """
    ...
    /* the page ground is the tray colour: if a browser leaves a strip below the game (Safari's
       toolbar area, the home-indicator zone), it reads as part of the tray, not as a white band */
    body { background: #d8c9a6; }
    .page { position: fixed; inset: 0; height: auto; max-width: none; padding: 0; margin: 0; background: #d8c9a6; }
    ...
    .tray { flex: 0 0 auto; padding: 5px 14px calc(5px + env(safe-area-inset-bottom)); grid-auto-rows: 40px; gap: 8px; border-top: none; }
    .stage, .phone, .screen { background: #d8c9a6; }
    .screen { background: #eef3e6; }
...
<html lang="nl">
  <head>
    ...
    <meta name="theme-color" content="#d8c9a6" />
